...
RSpec.describe ConvertSdk::MurmurHash3 do
  describe ".hash" do
    ...
    # These are NOT invented — each is a cross-SDK-proven expected value. They are
    # chosen here to exercise every trailing-byte (tail) branch: the body+tail
    # switch on len % 4 is where hand-ported Murmur implementations classically
    # diverge. byte-length % 4 noted per case.
    #
    ...
    VERIFIED_SEED9999 = {
      "" => 3_523_940_263,          # empty: skips body loop and tail switch entirely
      "hello" => 198_804_431,       # 1-byte tail
      "testString" => 2_241_850_228, # 2-byte tail (the cross-SDK anchor)
      "visitor-123" => 1_130_634_450 # 3-byte tail
    }.freeze

    VERIFIED_SEED9999.each do |input, expected|
      tail = input.bytes.length % 4
    ...
    it "always returns an unsigned 32-bit integer (0..2^32-1)" do
      %w[a ab abc abcd hello testString visitor-123].each do |key|
        [0, 1, 42, 9999, 2_147_483_647].each do |seed|
